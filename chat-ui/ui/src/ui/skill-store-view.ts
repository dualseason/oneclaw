...
function renderDetailPage(state: SkillStoreState, callbacks: SkillStoreCallbacks) {
  ...
  return html`
    <section class="skill-store__detail-page">
      <div class="skill-store__detail-topbar">
        <button
          class="skill-store__back-btn"
          type="button"
          @click=${callbacks.onCloseDetail}
        >← ${t("skillStore.back")}</button>
      </div>

      <div class="skill-store__detail-hero">
        <div class="skill-store__detail-hero-main">
          <div class="skill-store__detail-icon" style="background: ${bgColor}; color: #fff;">
            <span class="skill-store__card-letter">${letter}</span>
          </div>
          <div class="skill-store__detail-heading">
            <h3 class="skill-store__detail-title">${skill.name}</h3>
            <div class="skill-store__detail-slug">${skill.slug}</div>
            ${skill.description
              ? html`<p class="skill-store__detail-description">${skill.description}</p>`
              : nothing}
          </div>
        </div>

        <div class="skill-store__detail-actions">
          <button
            class="skill-store__btn skill-store__btn--ghost"
            type="button"
...
export function renderSkillStoreView(
  ...
  const showingDetail = Boolean(state.detailSlug);

  return html`
    ${state.error && !showingDetail
      ? html`<div class="skill-store__error">${state.error}</div>`
      : nothing}

    ${showingDetail
      ? renderDetailPage(state, callbacks)
      : html`
